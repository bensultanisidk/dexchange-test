...
export const AuthorsTable: FC = () => {
  ...
  const data = authorsData.map((author) => [
    <div className="flex items-center" key={author.email}>
      <div className="w-8 h-8 rounded-full bg-gray-200 mr-3 flex items-center justify-center text-lg">{author.avatar}</div>
      <div>
    ...
    author.employed,
    <a key="edit" href="#" className="text-gray-500 text-sm font-bold hover:text-gray-800 transition-colors">Edit</a>,
  ]);
...
export const ProjectsTable: FC = () => {
  const headers = ["COMPANIES", "BUDGET", "STATUS", "COMPLETION", ""];
